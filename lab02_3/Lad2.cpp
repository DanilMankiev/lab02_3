...
int main()
{
	...
	// Объясните назначение условия if (this == &v). Защита от самоприсваивания
	...
	stop
		...
		Vector c = a;
		// потому что вызывается конструктор копирования, а не оператор присваивания
	...
	MyString ss = s;
	// !нет потому что вызывается default конструктор копирования
...
// В пошаговом режиме с заходом в функции (F11) просмотрите ход выполнения.
	...
	Vector d;
	d = a + b; // вызывается еще и оператор присваивания
